...
from app.schemas.agent_outputs import MVPPlanOutput
from app.services.llm_factory import get_llm_provider
from app.prompts.agent_prompts import MVP_PLANNER_AGENT_PROMPT

logger = logging.getLogger(__name__)

class MVPPlannerAgent(BaseAgent):
    name: str = "MVP Planner Agent"
    ...
    def run(self, state: AgentState) -> AgentState:
        ...
        ctx = f"Startup Idea: '{idea}'\nInsights: {insights.model_dump_json() if insights else ''}\nCompetitors: {competitors.model_dump_json() if competitors else ''}"
        prompt = f"{rag_context}\nPlan an intentionally lean MVP roadmap based on context:\n{ctx}"
        
        result: MVPPlanOutput = llm.generate_structured(
            prompt=prompt,
            schema=MVPPlanOutput,
            system_prompt=MVP_PLANNER_AGENT_PROMPT
        )
        
        state["mvp_plan"] = result
